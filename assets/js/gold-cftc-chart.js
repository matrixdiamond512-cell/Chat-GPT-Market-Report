...
function buildSvg(history){
  ...
  const zero=py(0);
  const barW=Math.max(8,Math.min(24,pw/history.length*.58));
  ...
  const tickCandidates=[0,5,10,15,20,history.length-1].filter((v,i,a)=>v>=0&&v<history.length&&a.indexOf(v)===i);
  const xTicks=tickCandidates.map(idx=>{
    const weeks=history.length-1-idx;
    const label=idx===history.length-1?'今週':`${weeks}週前`;
  ...
  const netDots=netPts.map(p=>`<circle cx="${p[0]}" cy="${p[1]}" r="2.4" class="gold-cftc-point-net"/>`).join('');
  const priceDots=pricePts.map(p=>`<circle cx="${p[0]}" cy="${p[1]}" r="2.2" class="gold-cftc-point-price"/>`).join('');

  return `<svg viewBox="0 0 ${W} ${H}" class="gold-cftc-svg" role="img" aria-label="CFTC Managed Money 26週推移。Long、Short、Netと金価格を表示。"><text x="${m.l}" y="16" class="gold-cftc-axis-title">枚数（枚）</text><text x="${m.l+pw+10}" y="16" class="gold-cftc-axis-title">金価格（USD/oz）</text>${grid}<line x1="${m.l}" y1="${zero}" x2="${m.l+pw}" y2="${zero}" class="gold-cftc-zero"/>${bars}${netPath?`<path d="${netPath}" class="gold-cftc-net-line"/>${netDots}`:''}${pricePath?`<path d="${pricePath}" class="gold-cftc-price-line"/>${priceDots}`:''}${xTicks}${priceTicks}</svg>`;
}

function renderCard(cot){
  const history=Array.isArray(cot.historyWeeks)?cot.historyWeeks:[];
  ...
  const statusLabel=cot.historyStatus==='verified'?'26週取得済み':cot.historyStatus==='stale'?'前回26週履歴':'履歴取得待ち';

  return `<article class="gold-card gold-cftc-history-card" data-cftc-history-card><div class="gold-section-head"><h2 class="gold-section-title">投機筋ポジション推移</h2><span class="gold-frequency">週次・${esc(statusLabel)}</span></div><div class="gold-section-body"><p class="gold-cftc-history-intro">CFTC Managed Money の26週推移から、投機筋の買い越し・売り越しの変化と金価格との整合性を確認します。</p><div class="gold-cftc-kpis"><div class="gold-cftc-kpi"><div class="label">Net</div><div class="value ${cls(net)}">${signed(net,0,'枚')}</div><div class="sub">前週比 <span class="${cls(netChange)}">${signed(netChange,0,'枚')}</span></div></div><div class="gold-cftc-kpi"><div class="label">前週比</div><div class="value ${cls(netChange)}">${signed(netChange,0,'枚')}</div><div class="sub">${netPct===null?'—':signed(netPct,1,'%')}</div></div><div class="gold-cftc-kpi long"><div class="label">Long</div><div class="value">${fmt(long_,0)}枚</div><div class="sub">前週比 <span class="${cls(longChange)}">${signed(longChange,0,'枚')}</span></div></div><div class="gold-cftc-kpi short"><div class="label">Short</div><div class="value">${fmt(short,0)}枚</div><div class="sub">前週比 <span class="${cls(shortChange)}">${signed(shortChange,0,'枚')}</span></div></div><div class="gold-cftc-kpi judge"><div class="label">判定</div><div class="value">${esc(judgement(netChange))}</div><div class="sub">${esc(cot.judgement||'週次ポジションから判定')}</div></div></div>${history.length>=2?`<div class="gold-cftc-chart-shell">${buildSvg(history)}<div class="gold-cftc-legend"><span><i class="long"></i>Long（買い）</span><span><i class="short"></i>Short（売り）</span><span><i class="net"></i>Net（買い越し）</span><span><i class="price"></i>金価格（右軸）</span></div></div>`:`<div class="gold-cftc-empty">CFTC 26週時系列を取得中です。次回のゴールド需給更新後にグラフを表示します。</div>`}<div class="gold-cftc-read"><b>読み方</b><ul><li>Net上昇＝投機筋の買い越し拡大</li><li>Net下降＝買い越し縮小、または売り圧力の増加</li><li>金価格とNetが逆行する場合は、値動きとポジションの乖離に注意</li><li>CFTCは週次公表のため、日次データより遅行します</li></ul></div><div class="gold-cftc-foot"><span>出典：<a href="${esc(cot.sourceUrl||'https://www.cftc.gov/MarketReports/CommitmentsofTraders/index.htm')}" target="_blank" rel="noopener">${esc(cot.sourceName||'CFTC Commitments of Traders')}</a></span><span>単位：枚（COMEX Gold futures 1枚＝100トロイオンス）</span></div></div></article>`;
}
