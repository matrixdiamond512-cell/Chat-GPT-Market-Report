...
const weeklyRows=history=>[...new Map((history||[]).filter(x=>x.date).map(x=>[weekKey(x.date),x])).values()];
function legacyComparisonChart(history,period='52'){
 const counts={4:4,13:13,26:26,52:52,156:156},allRows=weeklyRows(history).filter(x=>n(x.buyBalance)!==null&&n(x.sellBalance)!==null&&n(x.nikkei225Close)!==null),rows=allRows.slice(-counts[period]);
 ...
 const width=1430,height=330,left=78,right=1360,top=42,bottom=258,plotW=right-left,plotH=bottom-top,values=rows.flatMap(x=>[n(x.buyBalance),n(x.sellBalance),n(x.buyBalance)-n(x.sellBalance)]),leftMin=Math.min(0,...values),leftMax=Math.max(0,...values),leftSpan=leftMax-leftMin||1,prices=rows.map(x=>n(x.nikkei225Close)),priceMin=Math.min(...prices),priceMax=Math.max(...prices),priceSpan=priceMax-priceMin||1;
 const x=i=>left+i*(plotW/(rows.length-1)),yLeft=v=>top+(leftMax-v)/leftSpan*plotH,yPrice=v=>top+(priceMax-v)/priceSpan*plotH,points=key=>rows.map((row,i)=>`${x(i)},${yLeft(key==='net'?n(row.buyBalance)-n(row.sellBalance):n(row[key]))}`).join(' '),pricePoints=rows.map((row,i)=>`${x(i)},${yPrice(n(row.nikkei225Close))}`).join(' '),zeroY=yLeft(0),netArea=`${left},${zeroY} ${points('net')} ${right},${zeroY}`;
 const yTicks=[0,.25,.5,.75,1].map(t=>{const y=top+t*plotH,leftValue=Math.round(leftMax-t*leftSpan),priceValue=Math.round(priceMax-t*priceSpan);return`<line class="comparison-grid" x1="${left}" y1="${y}" x2="${right}" y2="${y}"/><text class="comparison-axis left" x="${left-10}" y="${y+4}">${fmt(leftValue)}</text><text class="comparison-axis right" x="${right+10}" y="${y+4}">${fmt(priceValue)}</text>`}).join(''),labels=rows.map((row,i)=>`<text class="comparison-date" x="${x(i)}" y="292" text-anchor="end" transform="rotate(-45 ${x(i)} 292)">${esc(row.date.slice(5).replace('-','/'))}</text>`).join(''),latestRow=rows.at(-1),latestNet=n(latestRow.buyBalance)-n(latestRow.sellBalance),warnY=yLeft(500000),dangerY=yLeft(1000000);
 return`<section class="comparison-card"><div class="comparison-head"><div><span class="comparison-kicker">週次・複合チャート</span><h2 class="comparison-title">日経平均株価と裁定取引の残高</h2><p>買い残・売り残・差し引きと株価の方向を、同じ時間軸で比較します。</p></div><div class="comparison-controls"><div class="arb-periods" role="group" aria-label="複合チャート表示期間">${[['4','4週'],['13','13週'],['26','26週'],['52','52週'],['156','3年']].map(x=>`<button type="button" data-legacy-period="${x[0]}" aria-pressed="${x[0]===String(period)}">${x[1]}</button>`).join('')}</div><div class="comparison-latest"><span>最新ネット残高</span><b>${fmt(latestNet)}千株</b></div></div></div><div class="comparison-legend"><span class="legend-buy">裁定買い残</span><span class="legend-sell">裁定売り残</span><span class="legend-net">差し引き（買い残−売り残）</span><span class="legend-price">日経平均（右目盛）</span><span class="legend-kiyohara-warn">5億株・注意</span><span class="legend-kiyohara-danger">10億株・危険</span><small>${period==='156'&&allRows.length<156?`保存済み${allRows.length}週を表示`:`${period}週表示`}</small></div><div class="comparison-scroll"><svg class="comparison-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="日経平均株価と裁定取引残高の週次推移"><text class="comparison-unit" x="${left}" y="22">（千株）</text><text class="comparison-unit right" x="${right}" y="22">（円）</text>${yTicks}<line class="comparison-zero" x1="${left}" y1="${zeroY}" x2="${right}" y2="${zeroY}"/><line class="comparison-kiyohara-warn" x1="${left}" y1="${warnY}" x2="${right}" y2="${warnY}"/><text class="comparison-threshold warn" x="${left+10}" y="${warnY-7}">5億株・注意</text><line class="comparison-kiyohara-danger" x1="${left}" y1="${dangerY}" x2="${right}" y2="${dangerY}"/><text class="comparison-threshold danger" x="${left+10}" y="${dangerY-7}">10億株・危険</text><polygon class="comparison-net-area" points="${netArea}"/><polyline class="comparison-buy-line" points="${points('buyBalance')}"/><polyline class="comparison-sell-line" points="${points('sellBalance')}"/><polyline class="comparison-price-line" points="${pricePoints}"/>${labels}</svg></div></section>`;
}
function bindLegacyComparison(history){const host=root.querySelector('[data-legacy-comparison]');if(!host)return;const show=period=>{host.innerHTML=legacyComparisonChart(history,period);host.querySelectorAll('[data-legacy-period]').forEach(b=>b.addEventListener('click',()=>show(b.dataset.legacyPeriod)))};show('52')}
function legacyNetChart(title,rows,values,prices,tone){
 const left=72,right=1360,top=38,bottom=205,netMax=Math.max(1100000,...values)*1.08,pMin=Math.min(...prices),pMax=Math.max(...prices),pSpan=pMax-pMin||1,x=i=>left+i*((right-left)/(rows.length-1)),netY=v=>bottom-v/netMax*(bottom-top),priceY=p=>bottom-(p-pMin)/pSpan*(bottom-top),ticks=rows.map((row,i)=>`<text class="week-label" x="${x(i)}" y="242" text-anchor="end" transform="rotate(-45 ${x(i)} 242)">${esc(row.date.slice(5).replace('-','/'))}</text>`).join(''),netLine=values.map((v,i)=>`${x(i)},${netY(v)}`).join(' '),priceLine=prices.map((v,i)=>`${x(i)},${priceY(v)}`).join(' '),warnY=netY(500000),dangerY=netY(1000000);
 return`<article class="chart-card net-level-chart" style="--chart-tone:${tone}"><h2 class="card-title">${esc(title)}｜過去52週</h2><p class="chart-meta"><span class="legend net">━ ネット裁定残（左軸）</span><span class="legend price">━ 日経225終値（右軸）</span><span class="legend warn">--- 5億株・注意</span><span class="legend danger">--- 10億株・危険</span></p><svg class="chart combined-chart" viewBox="0 0 1430 260" role="img" aria-label="ネット裁定残と清原ライン、日経225終値"><line class="grid" x1="${left}" y1="${top}" x2="${right}" y2="${top}"/><line class="grid" x1="${left}" y1="${bottom}" x2="${right}" y2="${bottom}"/><line class="kiyohara-warn" x1="${left}" y1="${warnY}" x2="${right}" y2="${warnY}"/><line class="kiyohara-danger" x1="${left}" y1="${dangerY}" x2="${right}" y2="${dangerY}"/><text class="threshold-label warn" x="${left+8}" y="${warnY-5}">5億株・注意</text><text class="threshold-label danger" x="${left+8}" y="${dangerY-5}">10億株・危険</text><text x="${left}" y="22">ネット裁定残（千株）</text><text x="${right}" y="22" text-anchor="end">日経225終値（円）</text><text x="${left-8}" y="${top+4}" text-anchor="end">${fmt(Math.round(netMax))}</text><text x="${left-8}" y="${warnY+4}" text-anchor="end">500,000</text><text x="${left-8}" y="${dangerY+4}" text-anchor="end">1,000,000</text><text x="${left-8}" y="${bottom}" text-anchor="end">0</text><text x="${right+8}" y="${top+4}">${fmt(Math.round(pMax))}</text><text x="${right+8}" y="${bottom}">${fmt(Math.round(pMin))}</text><polyline class="net-balance-line" points="${netLine}"/><polyline class="price-line" points="${priceLine}"/><circle cx="${x(values.length-1)}" cy="${netY(values.at(-1))}" r="4" fill="${tone}"/>${ticks}</svg></article>`;
}
function legacyChart(title,history,key,tone){
 const valueFor=row=>key==='net'?n(row.buyBalance)-n(row.sellBalance):n(row[key]),rows=weeklyRows(history).filter(x=>valueFor(x)!==null&&n(x.nikkei225Close)!==null);
 ...
 const values=rows.map(valueFor),deltas=values.map((v,i)=>i?v-values[i-1]:0),prices=rows.map(x=>n(x.nikkei225Close));if(key==='net')return legacyNetChart(title,rows,values,prices,tone);
 const left=72,right=1360,top=38,bottom=205,zero=122,span=Math.max(...deltas.map(Math.abs))||1,pMin=Math.min(...prices),pMax=Math.max(...prices),pSpan=pMax-pMin||1,x=i=>left+i*((right-left)/(rows.length-1)),priceY=p=>bottom-(p-pMin)/pSpan*(bottom-top),barWidth=Math.max(5,(right-left)/rows.length*.56),bars=deltas.map((d,i)=>{if(!i)return'';const h=Math.abs(d)/span*72,y=d>=0?zero-h:zero;return`<rect x="${x(i)-barWidth/2}" y="${y}" width="${barWidth}" height="${h}" rx="2" fill="${d>=0?'#078f80':'#df4e52'}"/>`}).join(''),ticks=rows.map((row,i)=>`<text class="week-label" x="${x(i)}" y="242" text-anchor="end" transform="rotate(-45 ${x(i)} 242)">${esc(row.date.slice(5).replace('-','/'))}</text>`).join(''),line=prices.map((p,i)=>`${x(i)},${priceY(p)}`).join(' '),midPrice=Math.round((pMin+pMax)/2);
 return`<article class="chart-card" style="--chart-tone:${tone}"><h2 class="card-title">${esc(title)}｜過去52週</h2><p class="chart-meta"><span class="legend up">■ 前週比プラス</span><span class="legend down">■ 前週比マイナス</span><span class="legend price">━ 日経225終値（右軸）</span></p><svg class="chart combined-chart" viewBox="0 0 1430 260" role="img" aria-label="${esc(title)}の前週比と日経225終値"><line class="grid" x1="${left}" y1="${top}" x2="${right}" y2="${top}"/><line class="grid" x1="${left}" y1="${zero}" x2="${right}" y2="${zero}"/><line class="axis" x1="${left}" y1="${bottom}" x2="${right}" y2="${bottom}"/><text x="${left}" y="22">前週比（千株）</text><text x="${right}" y="22" text-anchor="end">日経225終値（円）</text><text x="${left-8}" y="${top+4}" text-anchor="end">+${fmt(Math.round(span))}</text><text x="${left-8}" y="${zero+4}" text-anchor="end">0</text><text x="${left-8}" y="${bottom}" text-anchor="end">-${fmt(Math.round(span))}</text><text x="${right+8}" y="${top+4}">${fmt(Math.round(pMax))}</text><text x="${right+8}" y="${zero+4}">${fmt(midPrice)}</text><text x="${right+8}" y="${bottom}">${fmt(Math.round(pMin))}</text>${bars}<polyline class="price-line" points="${line}"/><circle cx="${x(rows.length-1)}" cy="${priceY(prices.at(-1))}" r="4" fill="#173f7a"/>${ticks}</svg></article>`;
}
function analyze(d){
...
function render(d,supply,stocks){
 ...
 <section class="arb-legacy"><div class="dashboard-grid">${legacyChart('裁定買い残',d.history,'buyBalance','#2865e8')}${legacyChart('裁定売り残',d.history,'sellBalance','#7b43c5')}${legacyChart('ネット裁定残',d.history,'net','#078f80')}</div><div data-legacy-comparison></div></section>
 ...
 bindLegacyComparison(d.history);
}
