...
function legacyComparisonChart(history){
 const rows=weeklyRows(history).filter(x=>n(x.buyBalance)!==null&&n(x.sellBalance)!==null&&n(x.nikkei225Close)!==null);
...
function render(d,supply,stocks){
 ...
 <section class="arb-legacy"><div class="arb-legacy-heading"><h2>従来の4グラフ</h2><p>これまでの表示を変更せず、週次の増減と日経225終値を確認できます。</p></div><div class="dashboard-grid">${legacyChart('裁定買い残',d.history,'buyBalance','#2865e8')}${legacyChart('裁定売り残',d.history,'sellBalance','#7b43c5')}${legacyChart('ネット裁定残',d.history,'net','#078f80')}</div>${legacyComparisonChart(d.history)}</section>
