...
if(requestedDate){
  window.__ARBITRAGE_HISTORY_MODE__=true;
  window.fetch=async function(input,init){
    const u=urlOf(input);
    if(u?.pathname?.endsWith('/data/nikkei225-arbitrage.json')){
      const bundle=await getBundle();
      return new Response(JSON.stringify(bundle?.arbitrage||{}),{
        status:200,
...
function fmt(v){
  if(!v)return'取得不能';
  const s=String(v).slice(0,10).replaceAll('/','-');
...
async function setupCalendar(){
  const shell=document.querySelector('[data-arbitrage-history]');
  const input=document.getElementById('arbitrageHistoryDate');
  const prev=document.getElementById('arbitrageHistoryPrev');
  const next=document.getElementById('arbitrageHistoryNext');
  const status=document.getElementById('arbitrageHistoryStatus');
  ...
  try{
    ...
    status.textContent=`${live?'最新表示':'過去表示'}｜裁定データ ${fmt(current.asOfDate)}｜日経225終値 ${fmt(current.latestHistoryDate)}`;
