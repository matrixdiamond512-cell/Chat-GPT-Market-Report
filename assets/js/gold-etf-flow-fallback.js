...
function barSvg(rows){
  ...
  rows.forEach((row,i)=>{
    ...
    svg+=`<rect class="${row.value>=0?'bar-positive':'bar-negative'}" x="${cx-barW/2}" y="${top}" width="${barW}" height="${height}" rx="2"></rect>`;
    svg+=`<text class="value-text" x="${cx}" y="${valueY}" text-anchor="middle">${row.value>0?'+':''}${row.value.toFixed(2)}</text>`;
...
function cumulativeSvg(rows){
  ...
  let svg=`<div class="gold-etf-cum-total">${total>0?'+':''}${total.toFixed(2)}t</div><svg class="gold-etf-chart-svg" viewBox="0 0 ${W} ${H}" role="img" aria-label="GLD累積フロー折れ線グラフ">`;
  ...
  const first=points[0].date.slice(5).replace('-','/'),last=points[points.length-1].date.slice(5).replace('-','/');
