(() => {
  ...
  function verifiedUsBreadth(report) {
    ...
    if (payload?.status !== "verified" || !nyse || !nasdaq) return null;
    if (report?.date && payload.marketDate && payload.marketDate > report.date) return null;
    const advancers = Number(nyse.advancers || 0) + Number(nasdaq.advancers || 0);
    const decliners = Number(nyse.decliners || 0) + Number(nasdaq.decliners || 0);
    const unchanged = Number(nyse.unchanged || 0) + Number(nasdaq.unchanged || 0);
    ...
    return {
      ...
      asOf: payload.marketDate,
      source: "NYSE・NASDAQ市場内部データ",
      detail: {
        NYSE: nyse,
        NASDAQ: nasdaq,
        combinedAdvanceRate: payload.combinedAdvanceRate,
