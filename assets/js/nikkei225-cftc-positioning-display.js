...
function paint(card,sp){
 ...
 card.innerHTML=`<div class="nikkei-section-head"><h2 class="nikkei-section-title">10. 海外投機筋ポジション推移（CFTC・週次）</h2>${tools}</div><div class="nikkei-section-body">${body}</div>`;card.querySelectorAll('.nikkei-cftc-range button[data-weeks]').forEach(btn=>btn.addEventListener('click',()=>{activeWeeks=Number(btn.dataset.weeks)||52;paint(card,sp)}));
}
function render(sp){let card=root.querySelector('[data-cftc-positioning]');if(!card){const grid=findGrid();if(!grid)return false;card=document.createElement('article');card.className='nikkei-card nikkei-span-12 nikkei-positioning-card';card.setAttribute('data-cftc-positioning','');const ai=findAi();if(ai&&ai.parentElement===grid)grid.insertBefore(card,ai);else grid.appendChild(card);if(ai){const t=ai.querySelector('.nikkei-section-title');if(t&&/AI/.test(t.textContent||''))t.textContent=(t.textContent||'AI需給コメント').replace(/^\d+\./,'11.');}}currentCard=card;currentSp=sp;paint(card,sp);return true;}
injectStyles();
