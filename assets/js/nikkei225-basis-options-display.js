...
function card(re){return [...root.querySelectorAll('.nikkei-card')].find(x=>re.test(titleText(x)));}
function apply(d){
 ...
 const basisCard=card(/^(現物・先物ベーシス|現物終値 vs 先物)/);
 if(basisCard&&spot.status==='verified'&&fut.status==='verified'){
  const title=basisCard.querySelector('.nikkei-section-title');if(title)title.textContent='現物終値 vs 先物（単純差）';
 ...
 const optCard=card(/^(オプション・SQ需給|日経225オプション需給|オプション需給)/);
 if(optCard){
