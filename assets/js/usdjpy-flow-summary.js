...
const latest=(rows,key)=>rows.map(x=>x?.[key]).filter(Boolean).sort().at(-1);
const meta=(asOf,acquired)=>`<div class="usd-card-meta"><span>基準日 <b>${esc(fmt(asOf))}</b></span><span>取得日 <b>${esc(fmt(acquired))}</b></span></div>`;
...
function render(data){
  ...
  const realRows=topDrivers(real),specRows=topDrivers(spec),allRows=[...realRows,...specRows];
  const acquired=data.generatedAt||latest(allRows,'updatedAt');
  ...
  root.innerHTML=`<div class="usd-flow-head"><h2>実需・投機フローの要点</h2><p>方向判断に必要な主要材料を簡潔に表示</p></div>
    <div class="usd-flow-public-grid">
      <article class="usd-flow-public-card"><span>実需総合判定</span><strong class="${tone(realScore)}">${esc(realLabel)}</strong><ul>${evidence(real)}</ul>${meta(latest(realRows,'asOf'),acquired)}</article>
      <article class="usd-flow-public-card"><span>投機フロー</span><strong class="${tone(specScore)}">${esc(specLabel)}</strong><ul>${evidence(spec)}</ul>${meta(latest(specRows,'asOf'),acquired)}</article>
      <article class="usd-flow-public-card is-comment"><span>現在の読み方</span><strong>${esc(combined.leadingFlow&&combined.leadingFlow!=='保留'?`${combined.leadingFlow}主導`:'材料の一致を確認')}</strong><p>${esc(combined.sessionComment||'金利・出来高・オーダーの方向が一致するか確認します。')}</p>${meta(latest(allRows,'asOf'),acquired)}</article>
    </div>`;
