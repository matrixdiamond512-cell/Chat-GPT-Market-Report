(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .sector-mover-heading{display:flex;align-items:center;justify-content:space-between;margin:0 0 5px;font-size:14px;font-weight:1000}
  ...
  function metricRow(name, item) {
    ...
    return `<tr data-stock-metric="${esc(name)}">
      <td>${esc(name)}</td>
      <td class="num ${cls(value)}">${esc(value)}</td>
      <td class="num ${cls(change)}">${esc(change)}</td>
      <td class="comment">${esc(evaluation)}</td>
    </tr>`;
  }

  function applyNikkeiMetrics() {
    if (!nikkeiPayload?.metrics || requestedDate || applying) return false;
    const section = document.querySelector('section[aria-label="主要指数と市場内部"]');
    const panel = section?.querySelectorAll(":scope > .panel")?.[1];
    const tbody = panel?.querySelector("table tbody");
    if (!tbody) return false;

    const appliedKey = String(nikkeiPayload.generatedAt || nikkeiPayload.dataAsOf || "1");
    if (panel.dataset.nikkeiMetricsApplied === appliedKey && tbody.querySelector('tr[data-stock-metric="日経VI"]')) return true;

    applying = true;
    try {
      tbody.querySelectorAll("tr[data-stock-metric]").forEach(row => row.remove());
      Array.from(tbody.rows).forEach(row => {
        const name = row.cells[0]?.textContent.trim() || "";
        if (METRIC_ORDER.includes(name)) row.remove();
      });

      const viItem = nikkeiPayload.metrics["日経VI"] || {};
      const growthRow = Array.from(tbody.rows).find(row => row.cells[0]?.textContent.trim() === "グロース250");
      if (growthRow) growthRow.insertAdjacentHTML("afterend", metricRow("日経VI", viItem));
      else tbody.insertAdjacentHTML("afterbegin", metricRow("日経VI", viItem));

      const valuationNames = METRIC_ORDER.slice(1);
      const valuationHtml = valuationNames.map(name => metricRow(name, nikkeiPayload.metrics[name] || {})).join("");
      ...
      panel.dataset.nikkeiMetricsApplied = appliedKey;
      return true;
    } finally {
      applying = false;
    }
    ...
    return `<div class="sector-mover-group">
      ...
      ${list.map((item, index) => {
        ...
        return `<div class="sector-row">
          <span class="rank">${index + 1}</span><b>${esc(item.name)}</b>
  ...
  function applySectors() {
    if (!sectorPayload?.markets || requestedDate || applying) return false;
    ...
    ["us", "japan"].forEach((key, index) => {
      ...
      if (panel.dataset.sectorApplied === appliedKey) return;
  ...
  function injectCalendar() {
    ...
    const selected = requestedDate || historyIndex?.latestDate || entries[0]?.date || nikkeiPayload?.dataAsOf || "";
    ...
    control.innerHTML = `<button type="button" class="stocks-date-button" data-stock-prev ${older ? "" : "disabled"} aria-label="前の保存日">‹</button>
      <input type="date" class="stocks-date-input" data-stock-date value="${esc(selected)}" aria-label="表示日">
      <button type="button" class="stocks-date-button" data-stock-next ${newer ? "" : "disabled"} aria-label="次の保存日">›</button>`;
    root.parentNode.insertBefore(control, root);

    const status = document.createElement("div");
    status.className = "stocks-date-status";
    status.textContent = requestedDate ? `保存済み株式市場分析：${selected.replace(/-/g, "/")}` : entries.length ? `最新表示。保存済み履歴はカレンダーから選択できます（最新保存日 ${selected.replace(/-/g, "/")}）` : "最新表示。履歴データは次回保存からカレンダーに追加されます。";
  ...
  async function loadHistoricalPage() {
    ...
    try {
      ...
      if (!root) return;
      const latest = await loadJson("data/stocks.json").catch(() => null);
      const originalFetch = window.fetch;
      void originalFetch;
      root.innerHTML = `<div class="stocks-history-error">${requestedDate.replace(/-/g, "/")}の保存データを読み込みました。ページ全体の履歴表示は保存JSONに基づきます。</div>`;
      if (data?.marketInternals?.japan?.rows) {
        const section = document.createElement("section");
        section.className = "pair-grid";
        section.setAttribute("aria-label", "主要指数と市場内部");
        section.innerHTML = [data.marketInternals.us, data.marketInternals.japan].map(market => `<article class="panel"><h2 class="panel-title"><span class="flag">${flag(market?.flag)}</span>${esc(market?.title || "取得不能")}</h2><div class="table-wrap"><table class="market-table"><thead><tr>${arr(market?.columns).map(col => `<th>${esc(col)}</th>`).join("")}</tr></thead><tbody>${arr(market?.rows).map(row => `<tr>${arr(row).map((cell, idx) => `<td class="${idx === 1 || idx === 2 ? `num ${cls(cell)}` : idx === 3 ? "comment" : ""}">${esc(cell)}</td>`).join("")}</tr>`).join("")}</tbody></table></div></article>`).join("");
        root.appendChild(section);
      }
      document.querySelector("[data-updated]").textContent = `表示日：${requestedDate.replace(/-/g, "/")} / データ状態：履歴保存済み`;
      void latest;
    } catch (error) {
      if (root) root.innerHTML = `<div class="stocks-history-error">${requestedDate.replace(/-/g, "/")}の株式市場分析はまだ保存されていません。保存済みの日付を選択してください。</div>`;
    }
  }
  ...
  async function init() {
    ...
    applyLatestOverlays();
    const observer = new MutationObserver(() => applyLatestOverlays());
    observer.observe(document.querySelector("[data-stocks-root]") || document.body, { childList: true, subtree: true });
    let attempts = 0;
    ...
    setInterval(async () => {
      try { nikkeiPayload = await loadJson(NIKKEI_URL); } catch (_) {}
      try { sectorPayload = await loadJson(SECTOR_URL); } catch (_) {}
      applyLatestOverlays();
    }, 15 * 60 * 1000);
  }
