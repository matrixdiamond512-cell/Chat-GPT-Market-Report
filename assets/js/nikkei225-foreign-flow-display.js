...
function priceBounds(values){
  ...
  const pad=Math.max(300,(hi-lo)*.10);
  lo=Math.floor((lo-pad)/500)*500;
  hi=Math.ceil((hi+pad)/500)*500;
  return{lo,hi};
}

function labelEvery(weeks){
  if(weeks>=52)return 4;
  if(weeks>=26)return 2;
  return 1;
}

function svg(series,weeks){
  const W=1120,H=370,L=74,R=78,T=30,B=58;
  ...
  const zero=y(0);
  const group=plotW/series.length;
  const bar=Math.max(3,Math.min(18,group*.27));
  ...
  const parts=[`<svg class="nikkei-foreign-chart" viewBox="0 0 ${W} ${H}" role="img" aria-label="海外投資家の現物株・日経225先物ネットフローと日経225先物価格の${weeks}週推移">`];

  const latestX=L+(series.length-1)*group;
  parts.push(`<rect class="nikkei-foreign-now" x="${latestX}" y="${T}" width="${group}" height="${plotH}" rx="4"/>`);

  for(let i=0;i<=4;i++){
    const val=ymax-i*(2*ymax/4);
    const yy=y(val);
    parts.push(`<line class="nikkei-foreign-gridline" x1="${L}" y1="${yy}" x2="${W-R}" y2="${yy}"/>`);
    parts.push(`<text class="nikkei-foreign-ylabel" x="${L-10}" y="${yy+3}" text-anchor="end">${esc(fmt(val,0))}</text>`);
  }
  parts.push(`<line class="nikkei-foreign-zero" x1="${L}" y1="${zero}" x2="${W-R}" y2="${zero}"/>`);

  if(pb&&py){
    for(let i=0;i<=4;i++){
      const val=pb.hi-i*((pb.hi-pb.lo)/4),yy=py(val);
      parts.push(`<text class="nikkei-foreign-ylabel-right" x="${W-R+9}" y="${yy+3}" text-anchor="start">${esc(fmt(val,0))}</text>`);
    }
  }
  ...
  series.forEach((s,i)=>{
    ...
    const draw=(v,x,klass,label)=>{
      if(v===null)return;
      const yy=y(v),h=Math.abs(zero-yy),top=Math.min(zero,yy);
      parts.push(`<rect class="${klass}" x="${x-bar/2}" y="${top}" width="${bar}" height="${Math.max(1,h)}" rx="1.5"><title>${esc(date(s.asOfDate))} ${esc(label)} ${esc(signed(v,0,'億円'))}</title></rect>`);
    };
    draw(cash,cx-bar*.62,'nikkei-foreign-bar-cash','現物株');
    draw(fut,cx+bar*.62,'nikkei-foreign-bar-fut','日経225先物');

    const showLabel=i===0||i===series.length-1||i%every===0;
    if(showLabel){
      parts.push(`<text class="nikkei-foreign-xlabel ${i===series.length-1?'current':''}" x="${cx}" y="${H-28}" text-anchor="middle">${esc(i===series.length-1?'今週':shortDate(s.asOfDate))}</text>`);
    }
  });

  if(pb&&py){
    const points=series.map((s,i)=>{
      const p=num(s.nikkeiFuturesPrice);
    ...
    if(points.length>=2){
      ...
      series.forEach((s,i)=>{
        const p=num(s.nikkeiFuturesPrice);
        if(p===null)return;
        const cx=L+i*group+group/2,cy=py(p);
        if(weeks<=26||i===series.length-1){
          const r=weeks<=12?3.1:weeks<=26?2.4:3.1;
          parts.push(`<circle class="nikkei-foreign-price-point" cx="${cx}" cy="${cy}" r="${r}"><title>${esc(date(s.asOfDate))} 日経225先物 ${esc(fmt(p,0))}円</title></circle>`);
        }
      });
    }
  }

  parts.push(`<text class="nikkei-foreign-ylabel" x="${L}" y="16">差引（億円）</text>`);
  if(pb)parts.push(`<text class="nikkei-foreign-ylabel-right nikkei-foreign-axis-title-right" x="${W-R+4}" y="16">先物価格（円）</text>`);
  parts.push(`</svg>`);
  return parts.join('');
}

function periodButtons(){
  return `<div class="nikkei-foreign-period-switch" role="group" aria-label="表示期間">${PERIODS.map(w=>`<button type="button" data-foreign-period="${w}" aria-pressed="${selectedWeeks===w?'true':'false'}" class="${selectedWeeks===w?'active':''}">${w}週</button>`).join('')}</div>`;
}
...
function render(d){
  ...
  const full52=allSeries.length===52&&Number(f.historyWeeks)===52&&Number(f.priceHistoryWeeks)===52;
  const series=full52?allSeries.slice(-selectedWeeks):[];
  ...
  if(fut!==null&&priceChange!==null){
    ...
    insights.push('先物フローと価格の整合性は、同一基準日の価格取得後に判定。');
  }
  ...
  body.innerHTML=`
    ...
    ${full52?`
      <div class="nikkei-foreign-chart-panel">
        <div class="nikkei-foreign-chart-title-row">
          <div>
            <div class="nikkei-foreign-chart-title">週次ネットフロー＋日経225先物価格（過去${selectedWeeks}週間）</div>
            <div class="nikkei-foreign-chart-sub">同一基準日の現物・先物フローと、取引所発表の先物清算値（帳入値）を比較</div>
          </div>
          <div class="nikkei-foreign-chart-controls">
            ${periodButtons()}
            <div class="nikkei-foreign-chart-meta">
              <div class="nikkei-foreign-latest-price">先物 ${latestPrice===null?'取得待ち':`${fmt(latestPrice,0)}円`}</div>
              <div class="nikkei-foreign-freshness">基準日 ${esc(date(f.asOfDate))} / 52週検証済み</div>
            </div>
          </div>
        </div>
        <div class="nikkei-foreign-chart-scroll">${svg(series,selectedWeeks)}</div>
        <div class="nikkei-foreign-legend"><span><i class="cash"></i>現物株ネットフロー</span><span><i class="futures"></i>日経225先物ネットフロー</span><span><i class="price"></i>日経225先物価格（右軸）</span></div>
        <div class="nikkei-foreign-tooltip-note">※ 52週分の検証済みデータを保持。12週・26週・52週を表示切替できます。価格は同一基準日の清算値（帳入値）。現物と先物はヘッジ関係を含むため、合計フローは表示しません。</div>
      </div>`:
      `<div class="nikkei-foreign-empty">過去52週の同一期間データが揃っていません。取得できた期間だけで52週表示を装うことはせず、欠損週は推測で補いません。</div>`}
  ...
  body.querySelectorAll('[data-foreign-period]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      const w=Number(btn.getAttribute('data-foreign-period'));
      if(!PERIODS.includes(w)||w===selectedWeeks)return;
      selectedWeeks=w;
      render(currentData);
    });
  });
...
fetch(DATA_URL,{cache:'no-store'})
  ...
  .then(d=>{
    ...
    const timer=setInterval(()=>{
      if(render(d)||++n>80)clearInterval(timer);
    },100);
    render(d);
  })
  .catch(()=>{});
