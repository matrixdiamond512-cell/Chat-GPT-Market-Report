...
function svg(series){
  const W=1120,H=350,L=72,R=24,T=26,B=54;
  ...
  const parts=[`<svg class="nikkei-foreign-chart" viewBox="0 0 ${W} ${H}" role="img" aria-label="海外投資家の現物株と日経225先物の週次ネットフロー推移">`,`<rect class="nikkei-foreign-now" x="${nowX}" y="${T}" width="${group}" height="${plotH}" rx="5"/>`,...grid,`<line class="nikkei-foreign-zero" x1="${L}" y1="${zero}" x2="${W-R}" y2="${zero}"/>`];
  series.forEach((s,i)=>{
  ...
  parts.push(`<text class="nikkei-foreign-ylabel" x="${L}" y="15">差引（億円）</text>`,`</svg>`);
  return parts.join('');
}
function apply(d){
  ...
  if(cash*fut<0)insights.push('方向不一致のため、日本株全面強気・全面弱気とは断定しない。');
  else insights.push('方向一致が次週も続くかを確認し、単週だけで判断しない。');
  ...
  body.innerHTML=`
    ...
    ${weeks>=2?`<div class="nikkei-foreign-chart-panel"><div class="nikkei-foreign-chart-title-row"><div><div class="nikkei-foreign-chart-title">週次ネットフロー推移（直近${esc(String(weeks))}週）</div><div class="nikkei-foreign-chart-sub">同一期間の東証プライム現物と日経225先物を比較</div></div><div class="nikkei-foreign-freshness">基準日 ${esc(date(f.asOfDate))} / ${esc(f.historyStatus||'verified')}</div></div><div class="nikkei-foreign-chart-scroll">${svg(series)}</div><div class="nikkei-foreign-legend"><span><i class="cash"></i>現物株ネットフロー</span><span><i class="futures"></i>日経225先物ネットフロー</span></div><div class="nikkei-foreign-tooltip-note">※ 合計フローは表示しません。現物と先物はヘッジ関係を含むため、単純合算を総合需給とみなさないためです。</div></div>`:`<div class="nikkei-foreign-empty">週次推移は現在${esc(String(weeks))}週分。2週以上の同一期間データが揃い次第、推移グラフを表示します。欠損週は推測で補いません。</div>`}
    <div class="nikkei-foreign-insight"><h3>このグラフから読むこと</h3><ul>${insights.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div>
    <div class="nikkei-foreign-source-row">${source(f.cashSourceFileUrl,'JPX 東証プライム投資部門別売買状況',f.asOfDate)}${source(f.derivativesSourceFileUrl,'JPX 日経225先物 投資部門別取引状況',f.asOfDate)}</div>`;
  return true;
}
