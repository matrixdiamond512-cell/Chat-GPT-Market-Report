...
function render(g,market){
 ...
 <div class="gold-content-grid">
  ...
  <aside class="gold-stack">
   ...
   <article class="gold-card"><div class="gold-section-head"><h2 class="gold-section-title">中央銀行</h2><span class="gold-frequency">月次</span></div><div class="gold-section-body"><div class="gold-central"><div><b>世界中央銀行の純購入量</b><div class="gold-note">基準月 ${esc(cb.period||'取得待ち')} / 短期シグナルとは分離</div></div><div class="gold-central-main">${esc(cbValue)}</div></div>${source(cb)}<div class="gold-central"><div><b>中国人民銀行（PBoC）</b><div class="gold-note">金保有量 ${esc(pbcHoldings)} / ${esc(pbcStreak)} / 上期累計 ${esc(pbcYtd)} ${badge(pbc.status)}</div></div><div class="gold-central-main">${esc(pbcPurchaseLabel)} ${esc(pbcMonthly)}</div></div>${source(pbc)}<div class="gold-note">中国の金輸入量は税関統計で別管理し、人民銀行の購入量とは合算しません。</div></div></article>
