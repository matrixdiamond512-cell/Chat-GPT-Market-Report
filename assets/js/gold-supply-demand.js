...
function render(g,market){
 ...
 const c=g.comex||{}, cot=g.cftc||{}, etf=g.etf||{}, physical=g.physical||{}, curve=g.curve||{}, cb=g.centralBank||{}, pbc=cb.pBoC||cb.pboc||{}, env=g.environment||{}, a=g.assessment||{}, ds=g.dataStatus||{};
 ...
 const pbcHoldings=n(pbc.holdingsTonnes)!==null?`${fmt(pbc.holdingsTonnes,0)}t`:'取得待ち';
 const pbcMonthly=n(pbc.monthlyPurchaseTonnes)!==null?`${signed(pbc.monthlyPurchaseTonnes,0,'t')}`:'取得待ち';
 const pbcYtd=n(pbc.ytdPurchaseTonnes)!==null?`${signed(pbc.ytdPurchaseTonnes,0,'t')}`:'取得待ち';
 const pbcStreak=pbc.consecutivePurchaseMonths?`${pbc.consecutivePurchaseMonths}カ月連続`:'取得待ち';
 const pbcPurchaseLabel=pbc.period?`${String(pbc.period).replace('-', '年')}月購入`:'月次購入';
 const cbValueClass=n(cb.netPurchasesTonnes)!==null?'up':'';
 const pbcValueClass=n(pbc.monthlyPurchaseTonnes)!==null?'up':'';
 root.innerHTML=`
 <section class="gold-overview" aria-label="需給サマリー">
  ...
  <article class="gold-card gold-summary-card ${/買い/.test(a.shortTerm||'')?'positive':/売り/.test(a.shortTerm||'')?'negative':'neutral'}"><div class="gold-summary-label">短期需給</div><div class="gold-summary-value small ${/買い/.test(a.shortTerm||'')?'up':/売り/.test(a.shortTerm||'')?'down':''}">${esc(a.shortTerm||'判定待ち')}</div><div class="gold-summary-sub">COMEX・CFTC・ETF・実質金利・ドルを合成</div></article>
  <article class="gold-card gold-summary-card ${/強い|買い需要/.test(a.structural||'')?'positive':/弱い/.test(a.structural||'')?'negative':'neutral'}"><div class="gold-summary-label">構造的需給</div><div class="gold-summary-value small ${/強い|買い需要/.test(a.structural||'')?'up':/弱い/.test(a.structural||'')?'down':''}">${esc(a.structural||'判定待ち')}</div><div class="gold-summary-sub">中央銀行・中国/インド現物需要を短期と分離</div></article>
 ...
 <div class="gold-content-grid">
  ...
  <aside class="gold-stack">
   ...
   <article class="gold-card"><div class="gold-section-head"><h2 class="gold-section-title">中央銀行</h2><span class="gold-frequency">月次</span></div><div class="gold-section-body"><div class="gold-central"><div><b>世界中央銀行の純購入量</b><div class="gold-note">基準月 ${esc(cb.period||'取得待ち')} / 短期シグナルとは分離</div></div><div class="gold-central-main ${cbValueClass}">${esc(cbValue)}</div></div>${source(cb)}<div class="gold-central"><div><b>中国人民銀行（PBoC）</b><div class="gold-note">金保有量 ${esc(pbcHoldings)} / ${esc(pbcStreak)} / 上期累計 ${esc(pbcYtd)} ${badge(pbc.status)}</div></div><div class="gold-central-main ${pbcValueClass}">${esc(pbcPurchaseLabel)} ${esc(pbcMonthly)}</div></div>${source(pbc)}<div class="gold-note">中国の金輸入量は税関統計で別管理し、人民銀行の購入量とは合算しません。</div></div></article>
