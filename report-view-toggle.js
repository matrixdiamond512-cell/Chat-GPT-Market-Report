(() => {
  ...
  function readerMarket(market) {
    return `<section class="report-reader-market">
      <div class="report-reader-market-title">
  ...
  function renderSources(items = []) {
    ...
    return `<ul class="report-reader-sources">${rows.map(source => {
      ...
      return source.url
        ? `<li><a href="${esc(source.url)}" target="_blank" rel="noopener noreferrer">${label}</a>${note}</li>`
        : `<li>${label}${note}</li>`;
    }).join('')}</ul>`;
  }
  ...
  function renderFullText(report) {
    ...
    latest.innerHTML = `<article class="report-reader">
      <header class="report-reader-header">
        <p class="report-reader-kicker">MARKET REPORT · FULL TEXT</p>
        <h1>${esc(title)}</h1>
        <p class="report-reader-meta">${esc(dateLabel)}｜全文連続表示</p>
      </header>
      <nav class="report-reader-toc" aria-label="本文内目次">
        <a href="#reader-theme">相場テーマ</a>
        <a href="#reader-news">重要ニュース</a>
        <a href="#reader-flow">資金フロー</a>
        <a href="#reader-markets">個別市場</a>
        <a href="#reader-scenario">シナリオ</a>
        <a href="#reader-risk">リスク</a>
      </nav>
      ...
      <section class="report-reader-section" id="reader-markets">
        <h2>11. 個別市場見通し</h2>
        <div class="report-reader-markets">${visibleMarkets.length ? visibleMarkets.map(readerMarket).join('') : '<p class="report-reader-empty">該当市場の記載がありません。</p>'}</div>
      </section>
    ...
    latest.innerHTML = `
      <article class="report-summary-dashboard">
        <header class="report-summary-head">
          <div><span class="report-summary-kicker">INFOGRAPHIC DASHBOARD</span><h3>${esc(title)}</h3></div>
  ...
  function renderActiveView() {
    ...
    if (!report) return;
    if (activeView === 'summary') renderSummary(report);
    else renderFullText(report);
    tabs.forEach(tab => {
  ...
  const waitForInitialReport = setInterval(() => {
    if (!currentReport()) return;
    clearInterval(waitForInitialReport);
    renderActiveView();
  }, 100);
