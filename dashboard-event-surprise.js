(() => {
  ...
  const marketRead = (event, state) => {
    ...
    if (state === 'pending' || state === 'neutral') {
      return state === 'pending' ? '発表後に市場反応を確認' : '予想通りで、初動は需給や同時刻の材料が主導しやすい';
    }
    const up = state === 'positive';
    if (/CPI|PCE|PPI|雇用|失業|賃金|GDP|小売|ISM|PMI/i.test(title)) {
      return up ? '米金利・ドル上昇、株・金・BTCには逆風となる可能性' : '米金利・ドル低下、株・金・BTCには追い風となる可能性';
    }
    if (/原油|EIA|API|在庫/i.test(title)) {
      return up ? '在庫増なら原油に下押し圧力。需要指標の場合は内容を個別確認' : '在庫減なら原油に上昇圧力。需要指標の場合は内容を個別確認';
    }
  ...
  const reactionText = (event, key) => {
    ...
    const aliases = {
      initial:['initial','immediate','firstMove','initialReaction','atRelease'],
      m30:['m30','after30m','thirtyMinutes','reaction30m'],
      hours:['hours','afterHours','after2h','later','followThrough']
    };
  ...
  const reactionRow = event => {
    ...
    return `<div class="event-reaction-row">
      <span>初動<b>${esc(initial || '—')}</b></span>
      <span>30分後<b>${esc(m30 || '—')}</b></span>
      <span>数時間後<b>${esc(hours || '—')}</b></span>
    </div>`;
  };
  ...
  const consistencyRead = event => {
    ...
    const initial = reactionText(event, 'initial');
    if (!initial) return '';
    const expected = marketRead(event, direction(event).state);
    return `理論上の反応：${expected}。実際の初動：${initial}`;
  };
  ...
  async function init() {
    try {
      ...
      const completedHtml = completed.length ? completed.map(e => {
        ...
        return `<article class="event-surprise-card is-${d.state}">
          <div class="surprise-head"><strong>${esc(e.title)}</strong><span>${d.label}</span></div>
          <div class="surprise-values"><span>予想<b>${fmt(e.forecast,e.unit)}</b></span><span>結果<b>${fmt(e.actual,e.unit)}</b></span><span>予想差<b>${diff}</b></span></div>
          <p>${esc(marketRead(e,d.state))}</p>
          ${reactionRow(e)}
          ${consistency ? `<p class="event-data-note"><strong>整合性：</strong>${esc(consistency)}</p>` : ''}
        </article>`;
