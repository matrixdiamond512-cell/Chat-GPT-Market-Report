...
def parse_oi(url:str):
    ...
    found={}
    # The current JPX workbook uses two horizontal product blocks. The left
    # block starts at column index 0 and the right block at index 7.
    # Columns within each block are: product, contract, volume, OI, OI change,
    # previous OI. The exact cell positions were verified against the current
    # 2026-08-07 workbook before enabling the mini parser.
    blocks=(0,7)
    for ws in wb.worksheets:
        state={base:None for base in blocks}
        accum={}
...
def main():
    ...
    try:
        ...
        if mini and mini.get('openInterest') is not None:
            ...
            f['miniVolume']=None; f['miniOpenInterest']=None; f['miniOpenInterestChange']=None; f['miniPreviousOpenInterest']=None
    ...
    d['sourceStatus']=f'{connected}/10項目連携（基準日を個別表示）'; d.setdefault('diagnostics',{})['statuses']=statuses; d['diagnostics']['oiParser']='verified JPX horizontal blocks at columns 0 and 7'; d['generatedAt']=u.now()
