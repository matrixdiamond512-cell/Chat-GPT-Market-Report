(() => {
  ...
  function normalizeEvent(event) {
    ...
    const raw = event?.importance ?? event?.priority ?? event?.level ?? event?.rank ?? (HIGH_IMPACT.test(title) ? 3 : 2);
    let importance = Number(raw);
    ...
    return {time:event?.time || event?.jst || event?.datetime || event?.dateTime || "—",title,importance:Math.max(1,Math.min(3,importance)),impact:asArray(impact)};
  }
  ...
  function recurringEvents(dateString) {
    ...
    const events = [];
    if ([5,10,15,20,25,30].includes(day) && weekday >= 1 && weekday <= 5) events.push({time:"09:55",title:"東京仲値・ゴトー日需給",importance:2,impact:["USD/JPY"]});
    const nextDay = new Date(date); nextDay.setDate(day + 1);
    if (nextDay.getMonth() !== date.getMonth() && weekday >= 1 && weekday <= 5) events.push({time:"終日",title:"月末フロー・リバランス",importance:2,impact:["USD/JPY","EUR/USD","日経225先物","金"]});
  ...
  function extractEvents(report) {
    ...
    const textSources = [...asArray(report?.handover),...asArray(report?.news),...asArray(report?.changes),...asArray(report?.riskManagement)].map(itemText).filter(Boolean);
    const inferred = [];
    textSources.forEach(text => {
      if (!EVENT_WORDS.test(text)) return;
      text.split(/[。\n]|(?=\d{1,2}:\d{2})/).map(x => x.trim()).filter(Boolean).filter(chunk => EVENT_WORDS.test(chunk)).forEach(chunk => {
    ...
    const seen = new Set();
    return [...explicit,...inferred,...recurringEvents(report?.date)].map(normalizeEvent).filter(event => {
  ...
  function renderEvents(events) {
    ...
    if (!sorted.length) {
      container.innerHTML = '<p class="event-empty">選択した市場に該当するイベントはありません。</p>';
      return;
    }
    const now = new Date();
    container.innerHTML = sorted.slice(0,10).map(event => {
      ...
      return `<div class="event-row${status}"><time class="event-time">${escapeHtml(event.time)}</time><div class="event-main"><div class="event-title-row"><strong>${escapeHtml(event.title)}</strong><span class="event-level ${level.className}">${level.text}</span></div>${impact ? `<small>影響：${escapeHtml(impact)}</small>` : ""}</div></div>`;
    }).join("");
  }
  ...
  function updateCountdown() {
    ...
    if (!upcoming) {
      countdown.textContent = "本日の時刻指定イベントは終了しました";
      return;
    }
